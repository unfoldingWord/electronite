...
set BUILD_TARGET=.\src\out\Release-%TARGET_%\electron.exe
if exist %BUILD_TARGET% (
    echo "Build Target already exists: %BUILD_TARGET%"
) else (
    echo "Doing Build %TARGET_%"
    call .\electronite-tools-3.bat build %TARGET_%
...
set TARGET_FOLDER=.\src\out\Release-%TARGET_%
